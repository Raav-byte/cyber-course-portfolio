...
| Address | Class | Default Mask | Default CIDR |
|---|---|---|---|
...
The block is 10.10.0.0–10.10.0.63.
...
A /30 is useful for a point-to-point link because it gives 2 usable host addresses.
...
The /26 blocks are 0–63, 64–127, 128–191 and 192–255. 75 is in the 64–127 block, so it is a valid host.
...
The /25 blocks are 0–127 and 128–255. 130 is inside the second block and is not the network or broadcast address.
...
A /26 would fit all four departments, but it would waste address space for the smaller departments.
...
We need IPv6 because IPv4 does not have enough addresses for the number of devices connected to networks today. IPv6 uses 128-bit addresses, which gives us a much larger number of possible addresses.
